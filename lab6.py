...
data = [
    ...
    [round(p[i] + ((-1)**k)*m,3) for i in range(len(p))]
]
print("X:")
print(data[0])
print("Y:")
...
test1 = []
test1.append([i/10 + 0.03 for i in range(10)])
...
test2 = []
test2.append([i/10 + 0.07 for i in range(10)])
test2.append([newton_interpolation(data, x = test2[0][i]) for i in range(10)])

plt.scatter(data[0], data[1], color="yellow")
plt.scatter(test1[0], test1[1], color="lime")
plt.scatter(test2[0], test2[1], color="purple")
plt.show()